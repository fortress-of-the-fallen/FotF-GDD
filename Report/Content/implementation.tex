...
\begin{itemize}
    \item Luồng \textbf{Identity \& Profile}: đăng ký/\allowbreak đăng nhập, chọn profile/\allowbreak nhân vật, thiết lập phiên.
    \item Luồng \textbf{real-time}: client gửi input, server xử lý authoritative và trả snapshot/\allowbreak event.
...
\hline
M1 & Dựng backend, DB, auth và kết nối client & Register/\allowbreak Login, token, chọn profile \\
\hline
M2 & Dựng kênh real-time và room/\allowbreak instance runtime cơ bản & Join room, gửi input, nhận snapshot (di chuyển) \\
\hline
M3 & Tích hợp dữ liệu bền vững và mô-đun meta tối thiểu & Inventory/\allowbreak currency, gacha NPC, island resources, worker assignment \\
\hline
M4 & Hoàn thiện pipeline cấu hình và logging/\allowbreak audit tối thiểu & TSV import, config versioning, business logs \\
\hline
\end{tabularx}
...
\begin{itemize}
    \item \textbf{Presentation/\allowbreak UI}: màn hình login, profile/\allowbreak character select, HUD cơ bản, panel inventory/\allowbreak gacha/\allowbreak island.
...
Nguyên tắc chính:
\begin{itemize}
    \item Client không tự ý ``xác nhận'' các thay đổi tiến trình (reward/\allowbreak currency/\allowbreak gacha).
    \item Client hiển thị trạng thái dựa trên phản hồi server; với realtime, ưu tiên snapshot/\allowbreak event authoritative.
\end{itemize}
...
\begin{enumerate}
    \item \textbf{Login/\allowbreak Register}: nhập thông tin, gọi API, lưu token.
    \item \textbf{Profile Select}: liệt kê profile, tạo/\allowbreak chọn profile.
    \item \textbf{Character Select/\allowbreak Create}: liệt kê nhân vật theo profile, tạo/\allowbreak chọn nhân vật.
    \item \textbf{Main Hub (Island)}: load scene, hiển thị HUD, mở các panel inventory/\allowbreak gacha/\allowbreak NPC/\allowbreak island.
    \item \textbf{Join Instance (tuỳ mốc)}: tham gia room realtime để kiểm chứng đồng bộ di chuyển/\allowbreak combat đơn giản.
\end{enumerate}
...
\begin{itemize}
    \item \textbf{REST}: các thao tác quản lý (auth/\allowbreak profile/\allowbreak character/\allowbreak inventory/\allowbreak island/\allowbreak gacha).
    \item \textbf{WebSocket}: join room, gửi input, nhận snapshot/\allowbreak event.
\end{itemize}

Quy ước xử lý lỗi UI:
\begin{itemize}
    \item \textbf{401/\allowbreak 403}: token hết hạn hoặc sai quyền $\rightarrow$ điều hướng về login.
    \item \textbf{5xx}: lỗi hệ thống $\rightarrow$ hiển thị trạng thái retry.
    \item \textbf{network timeout}: hiển thị offline/\allowbreak reconnect và không cho thao tác tiến trình.
\end{itemize}
...
\begin{itemize}
    \item \textbf{AuthModule}: register/\allowbreak login, issue token, guard/\allowbreak role.
    ...
    \item \textbf{InventoryModule}: get inventory, apply delta (reward, currency), equip/\allowbreak enhance (tuỳ mốc).
    \item \textbf{GachaModule}: banner/\allowbreak rate/\allowbreak summon, update pity, duplicate handling.
    \item \textbf{IslandModule}: get island, build/\allowbreak upgrade, resource sync.
...
\begin{enumerate}
    \item Guard xác thực token và gắn context (userId/\allowbreak profileId/\allowbreak characterId nếu có).
    ...
    \item Ghi business log/\allowbreak audit log nếu là giao dịch quan trọng.
\end{enumerate}
...
\begin{itemize}
    \item \texttt{roomId}, \texttt{mode} (island/\allowbreak tower/\allowbreak dungeon/\allowbreak arena), danh sách player trong room.
    \item Runtime state: position, HP/\allowbreak MP, flags trạng thái, cooldown state tối thiểu.
...
\]
Các kỹ thuật nâng cao (prediction/\allowbreak reconciliation) được xem là hướng mở rộng sau khi luồng cơ bản ổn định.
...
\subsubsection{Schema collections và nguyên tắc embed/\allowbreak reference}
\label{subsubsec:impl-data-schema}

Các collections động (progression/\allowbreak state) ưu tiên embed khi dữ liệu gắn chặt vòng đời:
\begin{itemize}
    \item \textbf{CHARACTER}: nhúng base\_stats/\allowbreak resources/\allowbreak position/\allowbreak appearance.
...
\begin{itemize}
    ...
    \item \textbf{CONFIG\_BUILDING}: định nghĩa building và cost/\allowbreak time.
...
Các thao tác nhạy cảm (reward/\allowbreak currency/\allowbreak gacha/\allowbreak build upgrade) cần:
\begin{itemize}
    \item \textbf{Idempotency key}: mỗi request có mã định danh giao dịch để tránh cấp trùng khi retry.
    \item \textbf{Business log}: ghi lại delta trước/\allowbreak sau (hoặc sự kiện) để đối soát.
...
\begin{itemize}
    ...
    \item Kiểu dữ liệu rõ ràng (int/\allowbreak float/\allowbreak string/\allowbreak bool) theo template.
...
\begin{itemize}
    \item Auth: register/\allowbreak login, token expiry.
    \item Profile/\allowbreak Character: create/\allowbreak select, load summary.
    \item Realtime: join room, move input, snapshot update, disconnect handling.
    \item Gacha: summon 1x/\allowbreak 10x, pity update, duplicate handling, history.
    \item Island/\allowbreak NPC: build/\allowbreak upgrade timer (mock), assign worker, resource tick (mô phỏng).
\end{itemize}
...
\begin{itemize}
    \item Retry request (gacha/\allowbreak reward) không làm nhân đôi kết quả.
...
\begin{itemize}
    \item Có thể đăng nhập, chọn profile/\allowbreak nhân vật và vào scene hub.
    ...
    \item Island/\allowbreak NPC có luồng dữ liệu bền vững (lưu và tải lại đúng).
...
Chương này đã trình bày kế hoạch hiện thực prototype nhằm kiểm chứng thiết kế hệ thống: tổ chức client và server, luồng REST/\allowbreak WebSocket, room realtime tối thiểu, chiến lược lưu trữ dữ liệu, pipeline game configuration theo TSV và kế hoạch kiểm thử theo use case. Kết quả của chương là cơ sở để thực hiện đánh giá/\allowbreak đối chiếu trong chương tiếp theo khi prototype được triển khai và chạy thử.

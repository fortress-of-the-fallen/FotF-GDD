...
\begin{itemize}
    \item \textbf{Client} chịu trách nhiệm hiển thị, UI/\allowbreak UX, thu thập input và gửi yêu cầu/\allowbreak ý định hành động.
    \item \textbf{Server} chịu trách nhiệm xác thực, quản lý phiên, kiểm tra luật nghiệp vụ và xác nhận các thay đổi ảnh hưởng tiến trình (reward/\allowbreak currency/\allowbreak progression/\allowbreak gacha/\allowbreak island).
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Character \& Progression}: nhân vật, level/\allowbreak EXP, phân bổ chỉ số, mở slot kỹ năng, hệ race/\allowbreak class và điều kiện mở khoá.
    ...
    \item \textbf{Personal Island \& NPC}: đảo cá nhân, công trình, timer xây dựng/\allowbreak nâng cấp, tài nguyên, bộ sưu tập NPC và phân công worker.
    \item \textbf{Administration}: quản trị user, trạng thái, log/\allowbreak audit và cấu hình sự kiện.
\end{itemize}

Ngoài ra, hệ thống có lớp \textbf{game configuration} (dữ liệu tĩnh) phục vụ định nghĩa item/\allowbreak building/\allowbreak skill/\allowbreak rate, được quản lý theo pipeline riêng và được nạp vào \textit{Config Manager} để truy xuất nhất quán trong runtime.
...
\hline
Player & Người chơi sử dụng game client để đăng nhập, chọn profile/\allowbreak nhân vật, tham gia nội dung chiến đấu, quản lý tiến trình và tài sản. \\
\hline
Admin & Người quản trị hệ thống; có quyền thao tác trên user, log, mail hệ thống và cấu hình sự kiện. \\
\hline
Content Designer & Người thiết kế nội dung/\allowbreak thông số; quản lý bảng cấu hình (items/\allowbreak buildings/\allowbreak skills/\allowbreak rates) và xuất dữ liệu theo pipeline cấu hình. \\
\hline
Game Client & Ứng dụng chạy trên máy người chơi; hiển thị, gửi request HTTP, duy trì kênh real-time và hiển thị trạng thái. \\
\hline
Backend Server & Thành phần xử lý xác thực, nghiệp vụ và đồng bộ; ghi dữ liệu vào DB và cung cấp giao tiếp cho client/\allowbreak admin tools. \\
\hline
...
Nhóm này bao gồm đăng ký/\allowbreak đăng nhập, quản lý thiết lập, tạo/\allowbreak chọn game profile và thiết lập session runtime để chuẩn bị vào phiên chơi.
...
Nhóm này tập trung vào tạo/\allowbreak chọn nhân vật, level/\allowbreak EXP, phân bổ chỉ số, quản lý kỹ năng theo slot, combo skill và mở khoá/\allowbreak chuyển class theo điều kiện.
...
Nhóm này mô tả hành trình tham gia nội dung (tower/\allowbreak dungeon/\allowbreak arena), vòng lặp chiến đấu real-time, xử lý kết phiên và ghi nhận reward/\allowbreak checkpoint.
...
Nhóm này mô tả quy trình xem banner/\allowbreak tỉ lệ, thực hiện summon, cập nhật pity, xử lý trùng lặp và ghi lịch sử.
...
Nhóm này mô tả đảo cá nhân theo mô hình base-building: xây/\allowbreak nâng cấp công trình theo timer, quản lý tài nguyên, và phân công NPC làm worker.
...
Nhóm này bao gồm các tác vụ quản trị: quản lý trạng thái user, ban/\allowbreak unban, gửi mail hệ thống, xem log/\allowbreak audit và cấu hình sự kiện.
...
\textbf{Luồng chính:}
\begin{enumerate}
    \item Player nhập username/\allowbreak email và mật khẩu.
...
\textbf{Ngoại lệ:}
\begin{itemize}
    \item Email/\allowbreak username trùng $\rightarrow$ trả lỗi trùng lặp.
...
\textbf{Hậu điều kiện:} Token hợp lệ được cấp; server ghi nhận \texttt{last\_login}; phiên runtime có thể tham chiếu profile/\allowbreak character khi Player vào game.
...
\begin{itemize}
    ...
    \item Token hết hạn/\allowbreak không hợp lệ ở các request sau $\rightarrow$ yêu cầu login lại.
\end{itemize}
...
\textbf{Mục tiêu:} Player tạo/\allowbreak chọn GAME\_PROFILE để tách tiến trình theo nhiều profile (server/\allowbreak slot) trong cùng user.
...
\begin{enumerate}
    ...
    \item Server kiểm tra hợp lệ (tên, ràng buộc race/\allowbreak class).
...
\textbf{Ngoại lệ:}
\begin{itemize}
    \item Tên không hợp lệ/\allowbreak trùng $\rightarrow$ trả lỗi và yêu cầu nhập lại.
    \item Race/\allowbreak Class không hợp lệ $\rightarrow$ trả lỗi ràng buộc.
\end{itemize}
...
\begin{enumerate}
    ...
    \item Nếu level đạt mốc mở slot, server cập nhật trạng thái slot (normal/\allowbreak combo).
...
\begin{enumerate}
    ...
    \item Player kéo/\allowbreak thả hoặc chọn kỹ năng để gán vào normal slots.
    \item Player chọn các kỹ năng thành phần để tạo combo, sau đó gán vào combo slot.
    \item Server kiểm tra điều kiện (slot mở, điều kiện class/\allowbreak race/\allowbreak level, quy tắc combo).
...
\textbf{Mục tiêu:} Player mở khoá class mới dựa trên ngưỡng chỉ số/\allowbreak trait/\allowbreak achievement và có thể chuyển class theo chính sách.

\textbf{Tác nhân chính:} Player.

\textbf{Tiền điều kiện:} CHARACTER tồn tại; có dữ liệu chỉ số và trạng thái trait/\allowbreak achievement.
...
\begin{enumerate}
    ...
    \item Server kiểm tra điều kiện (stat threshold, karma/\allowbreak affinity/\allowbreak luck/\allowbreak resistance, điều kiện tiến trình).
...
\textbf{Mục tiêu:} Ghi nhận item/\allowbreak currency sau khi hoàn thành nội dung; đảm bảo quy tắc stack và dung lượng inventory.
...
% ---------------------------
\subsubsection{UC-C01 -- Tham gia nội dung theo phiên (Tower/\allowbreak Dungeon/\allowbreak Arena)}
\label{subsubsec:uc-c01}

\textbf{Mục tiêu:} Player chọn nội dung, tham gia instance/\allowbreak room và bắt đầu vòng lặp combat real-time.
...
\textbf{Luồng chính:}
\begin{enumerate}
    \item Player chọn chế độ (tower/\allowbreak dungeon/\allowbreak arena) và tham số (tầng, độ khó, \ldots).
    \item Client gửi request/\allowbreak join event.
    \item Server kiểm tra điều kiện tham gia (level, tài nguyên, vé, \ldots).
    \item Server tạo hoặc gán vào instance, gửi thông tin spawn/\allowbreak room state.
...
\textbf{Hậu điều kiện:} Trạng thái authoritative được cập nhật; client hiển thị nhất quán theo snapshot/\allowbreak event.

\textbf{Luồng chính:}
\begin{enumerate}
    \item Client gửi input/\allowbreak intent (move, attack, cast skill, dodge) theo nhịp.
    \item Server xác thực input theo luật (cooldown, tài nguyên, vị trí hợp lệ).
    \item Server cập nhật trạng thái (HP/\allowbreak MP/\allowbreak buff/\allowbreak debuff/\allowbreak position) và phát snapshot/\allowbreak event.
    \item Client nội suy/\allowbreak trình diễn (animation, VFX) theo trạng thái nhận được.
\end{enumerate}
...
\textbf{Tiền điều kiện:} Phiên kết thúc (thắng/\allowbreak thua/\allowbreak thoát).
...
\begin{enumerate}
    ...
    \item Server cập nhật EXP/\allowbreak level (UC-P02), cập nhật inventory (UC-I01).
    \item Server cập nhật checkpoint (tower/\allowbreak dungeon) nếu đạt điều kiện.
...
\textbf{Hậu điều kiện:} Kết quả summon được ghi nhận; NPC/\allowbreak đơn vị mới thêm vào collection hoặc chuyển đổi theo rule; pity cập nhật nhất quán.

\textbf{Luồng chính:}
\begin{enumerate}
    \item Player chọn banner và hình thức summon (1x/\allowbreak 10x).
    \item Server kiểm tra tài nguyên và trừ chi phí.
    \item Server thực hiện RNG theo rate của banner; áp dụng pity nếu đạt ngưỡng.
    \item Với kết quả trùng: server chuyển đổi sang shard/\allowbreak point theo rule.
...
% ---------------------------
\subsubsection{UC-S01 -- Xây/\allowbreak Nâng cấp công trình trên đảo cá nhân}
...
\textbf{Hậu điều kiện:} BUILDING instance được tạo/\allowbreak cập nhật trạng thái; \texttt{finish\_time} được thiết lập; tài nguyên đảo được cập nhật.

\textbf{Luồng chính:}
\begin{enumerate}
    \item Player chọn loại công trình và vị trí grid.
    \item Client gửi request build/\allowbreak upgrade.
    \item Server kiểm tra vị trí hợp lệ, không chồng lấn, thoả điều kiện level/\allowbreak town hall.
    \item Server trừ tài nguyên theo \texttt{CONFIG\_BUILDING.cost} và đặt \texttt{finish\_time}.
    \item Client hiển thị trạng thái \textit{Constructing/\allowbreak Upgrading}.
\end{enumerate}
...
\textbf{Mục tiêu:} Player phân công NPC vào công trình để vận hành/\allowbreak đẩy tiến trình theo job.
...
% ---------------------------
\subsubsection{UC-AD01 -- Quản trị user: ban/\allowbreak unban và audit log}
\label{subsubsec:uc-ad01}

\textbf{Mục tiêu:} Admin quản lý trạng thái user, ban/\allowbreak unban và đảm bảo có audit log.
...
\textbf{Luồng chính:}
\begin{enumerate}
    \item Admin tìm kiếm user theo username/\allowbreak email/\allowbreak id.
    \item Admin thực hiện ban/\allowbreak unban hoặc thay đổi trạng thái.
    \item Server cập nhật USER.status và ghi audit log (actor, thời gian, hành động).
    \item (Tuỳ chính sách) Server thu hồi session runtime/\allowbreak token hiện hành.
\end{enumerate}
...
Class được phân tầng (Basic/\allowbreak Intermediate/\allowbreak Advanced/\allowbreak Legendary/\allowbreak Hidden). Điều kiện mở khoá có thể kết hợp:
\begin{itemize}
    \item Ngưỡng chỉ số (STR/\allowbreak DEX/\allowbreak CON/\allowbreak INT/\allowbreak WIS/\allowbreak CHA).
    \item Trait đặc biệt (Karma/\allowbreak Affinity/\allowbreak Luck/\allowbreak Resistance, \ldots).
    \item Điều kiện hành vi/\allowbreak tiến trình (ví dụ thắng PvP, tham gia arena, số NPC chỉ huy).
...
\begin{itemize}
    ...
    \item Xây/\allowbreak nâng cấp công trình tạo timer (\texttt{finish\_time}) và trạng thái (building status).
...
\begin{itemize}
    ...
    \item Kết quả trùng lặp được chuyển đổi theo rule (shard/\allowbreak point/\ldots) để tránh ``mất giá trị''.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Character}: \texttt{CHARACTER} (base\_stats/\allowbreak resources/\allowbreak position/\allowbreak appearance dạng nhúng).
    ...
    \item \textbf{NPC}: \texttt{NPC\_COLLECTION} và \texttt{STRUCT\_OWNED\_NPC} (tái sử dụng struct stats/\allowbreak resources).
\end{itemize}

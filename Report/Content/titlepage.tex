% ===================== Content/titlepage.tex =====================
\begin{titlepage}
\thispagestyle{empty}

\begin{tikzpicture}[remember picture, overlay]
  \draw[line width = 4pt] ($(current page.north west) + (0.4in,-0.5in)$)
        rectangle ($(current page.south east) + (-0.4in,0.5in)$);
  \draw[line width=1.5pt]
        ($(current page.north west) + (0.45in,-0.55in)$)
        rectangle
        ($(current page.south east) + (-0.45in,0.55in)$);
\end{tikzpicture}

\vspace{-2cm}
\begin{center}
    \fontsize{14}{16}\selectfont\textbf{ĐẠI HỌC QUỐC GIA TP. HỒ CHÍ MINH}\\[0.2cm]
    \fontsize{14}{16}\selectfont\textbf{TRƯỜNG ĐẠI HỌC BÁCH KHOA}\\[0.2cm]
    \fontsize{14}{16}\selectfont\textbf{KHOA KHOA HỌC VÀ KỸ THUẬT MÁY TÍNH}
\end{center}

\vspace{0.35cm}

\begin{center}
\includegraphics[width=5cm]{Images/hcmut.png}
\end{center}

\vspace{0.35cm}

\begin{center}
    \fontsize{16}{18}\selectfont\textbf{ĐỒ ÁN TỐT NGHIỆP}\\[0.2cm]
    \fontsize{16}{18}\selectfont\textbf{BÁO CÁO}
\end{center}

\vspace{0.65cm}

\begin{center}
    \setstretch{1.5}
    {\fontsize{18}{20}\selectfont\textbf{PHÁT TRIỂN HỆ THỐNG GAME}}\\[0.1cm]
    {\fontsize{18}{20}\selectfont\textbf{CHIẾN THUẬT GIẢI TRÍ NHẸ}}\\[0.3cm]
    {\fontsize{14}{16}\selectfont\textbf{``FORTRESS OF THE FALLEN''}}\\[0.55cm]
    {\fontsize{14}{16}\selectfont Ngành: Khoa học Máy tính}\\[0.2cm]
    {\fontsize{12}{14}\selectfont Học kỳ 2 năm học 2025--2026 (HK252)}\\[0.2cm]
    {\fontsize{12}{14}\selectfont Mã đồ án: HK252-DATN-409}
\end{center}

\vspace{0.45cm}

\begin{center}
\fontsize{12}{14}\selectfont
\renewcommand{\arraystretch}{1.2}

% Dòng hội đồng + giảng viên
\begin{tabular}{@{}L{3.2cm}L{10.5cm}@{}}
    \textbf{Hội đồng:} & Hội đồng 4L Khoa học Máy tính \\
    \textbf{Đợt:} & GD2 -- DATN/LVTN (Đồ án tốt nghiệp/Luận văn tốt nghiệp) -- HK252 \\
    \textbf{Giảng viên hướng dẫn:} & ThS. Vương Bá Thịnh \\
\end{tabular}


\vspace{0.25cm}

\begin{tabular}{@{}l c@{}}
    \textbf{Sinh viên thực hiện} & \textbf{MSSV} \\[0.15cm]
    Trần Minh Khang   & 2211472 \\
    Hà Thái Toàn      & 2213524 \\
\end{tabular}

\vspace{0.25cm}

\end{center}

\vspace{0.2cm}

\begin{center}
    \fontsize{13}{15}\selectfont TP. Hồ Chí Minh, năm 2026
\end{center}

\end{titlepage}

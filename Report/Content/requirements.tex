...
Chương này xác định yêu cầu cho hệ thống game \textit{Fortress of the Fallen} ở mức \textbf{phân tích hệ thống}. Nội dung bao gồm: phạm vi, tác nhân, nhóm yêu cầu chức năng theo phân hệ, yêu cầu phi chức năng và yêu cầu dữ liệu. Các yêu cầu ở chương này là cơ sở để xây dựng mô hình use case/\allowbreak luồng nghiệp vụ (Chương~6) và kiến trúc + thiết kế dữ liệu (Chương~7).
...
\begin{itemize}
    \item Hệ thống vận hành theo mô hình client--server; server giữ vai trò quyết định cho các kết quả quan trọng (reward/\allowbreak currency/\allowbreak progression).
    \item Dữ liệu trong hệ thống được chia thành:
    \begin{itemize}
        \item \textbf{Player progression/\allowbreak state (động)}: thay đổi theo hành vi người chơi.
        \item \textbf{Game configuration (tĩnh)}: định nghĩa và tham số gameplay (item/\allowbreak building/\allowbreak skill/\allowbreak rates/\ldots).
    \end{itemize}
...
\begin{itemize}
    \item \textbf{Client}: hiển thị (render), UI, input, điều hướng màn hình, nội suy hiển thị; gửi yêu cầu/\allowbreak ý định hành động.
    \item \textbf{Server}: xác thực, quản lý phiên, xử lý nghiệp vụ, xác nhận kết quả, ghi DB.
    \item \textbf{Data layer}: DB cho progression/\allowbreak state; collections cấu hình cho configuration; cache/\allowbreak session runtime (định hướng).
\end{itemize}
...
\begin{itemize}
    \item \textbf{FR-A01} -- Đăng ký tài khoản: Player có thể đăng ký bằng username/\allowbreak email và mật khẩu.
    ...
    \item \textbf{FR-A05} -- Tạo game profile: một user có thể tạo nhiều profile (phục vụ nhiều server/\allowbreak nhân vật).
    \item \textbf{FR-A06} -- Chọn game profile: chọn profile để vào game; cập nhật \texttt{last\_played}.
    \item \textbf{FR-A07} -- Quản lý session runtime: hệ thống lưu trạng thái phiên hiện hành (profile/\allowbreak character/\allowbreak room\_id).
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{FR-B03} -- Tuỳ chọn ngoại hình: lưu các tham số appearance (hair/\allowbreak skin/\allowbreak face) để tái hiện nhất quán.
...
\begin{itemize}
    ...
    \item \textbf{FR-C03} -- Level không tự động cấp kỹ năng; kỹ năng được thu thập từ nguồn khác (loot/\allowbreak quest/\allowbreak NPC/\ldots).
\end{itemize}

\paragraph{Stat allocation}
\begin{itemize}
    \item \textbf{FR-C04} -- Player có thể phân bổ điểm vào các thuộc tính nền: STR, DEX, CON, INT, WIS, CHA.
    \item \textbf{FR-C05} -- Hệ thống tính các chỉ số suy diễn (HP/\allowbreak MP/\allowbreak ATK/\allowbreak DEF/\allowbreak CRIT/\allowbreak ACC/\allowbreak EVA/\ldots) từ thuộc tính nền theo công thức cấu hình.
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item Combo slots: mở dần tại các mốc 40/\allowbreak 60/\allowbreak 80/\allowbreak 100 đến tối đa 4 slot (slot cuối là \textit{ultimate combo}).
    \end{itemize}
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{FR-C11} -- Player có thể trang bị kỹ năng vào slot; hệ thống kiểm tra điều kiện (class/\allowbreak race/\allowbreak level).
...
\begin{itemize}
    \item \textbf{FR-D01} -- Hỗ trợ nhiều nhánh class (Warrior/\allowbreak Archer/\allowbreak Mage/\allowbreak Healer/\allowbreak Rogue/\allowbreak Tactician).
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item điều kiện tiến trình/\allowbreak achievement (PvP, quest, sưu tầm, \ldots).
    \end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{FR-E03} -- Item trong inventory là \textbf{item instance} (uid/\allowbreak quantity/\allowbreak slot/\allowbreak enhancement/\allowbreak durability) và tham chiếu đến \textbf{item definition} qua \texttt{item\_ref\_id}.
    \item \textbf{FR-E04} -- Stack rule: item có \texttt{max\_stack} theo cấu hình.
    \item \textbf{FR-E05} -- Equip: trang bị vũ khí/\allowbreak giáp theo slot (theo cấu hình), cập nhật chỉ số hiệu lực.
    \item \textbf{FR-E06} -- Enhance: nâng cấp trang bị theo chi phí và quy tắc; ghi nhận \texttt{enhancement\_level}.
    \item \textbf{FR-E07} -- Durability (tuỳ chọn theo thiết kế): giảm độ bền theo sử dụng và có cơ chế sửa/\allowbreak khôi phục.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{FR-F06} -- Dodge/\allowbreak Dash.
...
\begin{itemize}
    \item \textbf{FR-F09} -- Checkpoint: lưu mốc tiến độ theo nội dung (tower/\allowbreak dungeon) theo chính sách.
    \item \textbf{FR-F10} -- Loot delivery: reward sau combat được ghi vào inventory; hệ thống kiểm tra capacity/\allowbreak stack.
    \item \textbf{FR-F11} -- Reward logging: các giao dịch reward/\allowbreak currency cần được ghi log nghiệp vụ để truy vết.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{FR-G07} -- Duplicate handling: xử lý trùng lặp theo rule (convert shard/\allowbreak point/\ldots).
...
\begin{itemize}
    ...
    \item \textbf{FR-H02} -- Quản lý tài nguyên đảo: wood/\allowbreak stone/\allowbreak food/\allowbreak iron (hoặc tập tương đương theo cấu hình).
    ...
    \item \textbf{FR-H06} -- Harvest: thu tài nguyên theo chu kỳ/\allowbreak điều kiện.
\end{itemize}

\paragraph{NPC collection và worker assignment}
\begin{itemize}
    \item \textbf{FR-H07} -- Mỗi GAME\_PROFILE sở hữu 1 bộ sưu tập NPC (1:1).
    \item \textbf{FR-H08} -- NPC có rarity/\allowbreak level/\allowbreak star/\allowbreak friendship/\allowbreak current\_job và có thể tái sử dụng cấu trúc stats/\allowbreak resources giống nhân vật.
...
\begin{itemize}
    \item \textbf{FR-I01} -- Quản lý user: xem thông tin, cập nhật trạng thái (active/\allowbreak banned/\ldots).
    \item \textbf{FR-I02} -- Ban/\allowbreak Unban: admin có thể ban/\allowbreak unban; thu hồi session theo chính sách.
    \item \textbf{FR-I03} -- System mail: gửi mail hệ thống theo user/\allowbreak profile/\allowbreak nhóm.
    \item \textbf{FR-I04} -- Log viewer: xem log theo thời gian/\allowbreak severity/\allowbreak actor.
    \item \textbf{FR-I05} -- Event config: tạo/\allowbreak sửa cấu hình sự kiện theo thời gian và phần thưởng (lưu DB).
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{NFR-S02} -- Token/\allowbreak session có thời hạn; hỗ trợ revoke khi ban user hoặc logout.
    \item \textbf{NFR-S03} -- Phân quyền admin theo role; mọi thao tác admin cần audit log.
    \item \textbf{NFR-S04} -- Các thao tác reward/\allowbreak currency/\allowbreak progression phải chống cấp trùng (idempotent ở mức thiết kế).
\end{itemize}

\subsubsection{Hiệu năng và tính phản hồi}

\begin{itemize}
    \item \textbf{NFR-P01} -- Các API không real-time (login/\allowbreak profile/\allowbreak inventory) phải có thời gian phản hồi ổn định.
    \item \textbf{NFR-P02} -- Kênh real-time cần hỗ trợ cập nhật trạng thái thường xuyên (snapshot/\allowbreak event) để client nội suy mượt.
...
\begin{itemize}
    ...
    \item \textbf{NFR-O02} -- Có khả năng truy vết sự cố theo user/\allowbreak profile/\allowbreak character và theo mốc thời gian.
\end{itemize}

\subsubsection{Yêu cầu UX/\allowbreak UI ở mức hệ thống}

\begin{itemize}
    \item \textbf{NFR-U01} -- UI phải hiển thị rõ trạng thái: loading/\allowbreak empty/\allowbreak error/\allowbreak success/\allowbreak disabled.
    \item \textbf{NFR-U02} -- Với phong cách pixel art, UI cần ưu tiên readability: khung panel rõ, icon theo lưới pixel, tương phản đủ.
    \item \textbf{NFR-U03} -- Combat HUD tối thiểu: HP/\allowbreak MP (hoặc tài nguyên tương đương), skill slots, cooldown, buff/\allowbreak debuff.
\end{itemize}
...
Bao gồm: user/\allowbreak profile/\allowbreak session, character, inventory, island state, npc collection, pity state, history logs.

\paragraph{Dữ liệu cấu hình game (static)}
Bao gồm: định nghĩa item/\allowbreak building/\allowbreak skill, bảng hệ số chỉ số, drop tables, banner rates, pity rules, event configs.
...
\begin{itemize}
    ...
    \item \textbf{DR-07} -- Building instance tham chiếu building config (type/\allowbreak id $\rightarrow$ CONFIG\_BUILDING.id).
\end{itemize}
...
\begin{itemize}
    ...
    \item Mô hình dữ liệu mức logic thể hiện rõ quan hệ user/\allowbreak profile/\allowbreak character/\allowbreak inventory/\allowbreak island/\allowbreak npc và mối liên kết với config.
    \item Có mô tả rõ về ranh giới client--server, đặc biệt với các thao tác reward/\allowbreak currency/\allowbreak gacha và luồng real-time combat.

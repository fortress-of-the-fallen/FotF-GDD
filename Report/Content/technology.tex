...
\begin{itemize}
    ...
    \item \textbf{Hỗ trợ đa giao thức}: REST là kênh đang vận hành; WebSocket gateway ở mức định hướng mở rộng cho đồng bộ realtime sâu\cite{nest-docs}.
...
\begin{itemize}
    ...
    \item Tải dữ liệu cấu hình và các thao tác quản trị/\allowbreak truy vấn lịch sử\cite{nest-docs}.
\end{itemize}

\subsubsection{WebSocket (định hướng mở rộng)}

WebSocket phù hợp cho các tương tác realtime trong phiên chơi khi mở rộng:
\begin{itemize}
...
\end{itemize}
Trong bản hiện tại, các luồng nghiệp vụ cốt lõi đang được kiểm chứng chủ yếu qua REST API; WebSocket chưa là kênh runtime chính thức và được giữ như lớp mở rộng cho các chế độ realtime sâu hơn (ví dụ đồng bộ room/\allowbreak PvP theo tick). Với mô hình server authoritative, WebSocket là kênh phù hợp để truyền nhận input/\allowbreak snapshot khi mở rộng\cite{moriarty-networked,gaffer-networking}.
...
\begin{itemize}
    \item \textbf{Dữ liệu tiến trình (dynamic)}: user/\allowbreak session/\allowbreak profile/\allowbreak statistics/\allowbreak tutorial/\allowbreak currency/\allowbreak inventory/\allowbreak formation/\allowbreak quests/\allowbreak battles cần lưu bền vững và truy vấn theo khoá định danh.
    \item \textbf{Dữ liệu cấu hình (static)}: các collection cấu hình như hero/\allowbreak quest/\allowbreak rank/\allowbreak upgrade/\allowbreak account-level/\allowbreak campaign có thể được import từ pipeline TSV và nạp vào bộ nhớ khi chạy.
\end{itemize}

\subsubsection{Redis}

Redis được sử dụng ở mức hỗ trợ hiệu năng và phối hợp realtime theo nhu cầu triển khai\cite{redis-in-action}:
\begin{itemize}

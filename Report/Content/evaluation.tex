...
\begin{itemize}
    \item \textbf{Đối chiếu bao phủ yêu cầu (Requirement Coverage)}: kiểm tra các yêu cầu chức năng/\allowbreak phi chức năng đã được ánh xạ tới mô-đun, dữ liệu và luồng giao tiếp hay chưa.
    ...
    \item \textbf{Phân tích rủi ro và hướng giảm thiểu}: xác định các rủi ro kỹ thuật/\allowbreak thiết kế, mức ảnh hưởng và biện pháp kiểm soát.
\end{itemize}
...
\hline
\textbf{Nhóm FR} & \textbf{Phân hệ} & \textbf{Mô-đun/\allowbreak Thiết kế đáp ứng} \\
\hline
FR-A & Auth \& Profile & AuthModule, ProfileModule, SessionRuntime (token, profile\_id, socket\_id) \\
\hline
FR-B & Character & CharacterModule (create/\allowbreak select/\allowbreak delete), dữ liệu nhúng appearance/\allowbreak base\_stats/\allowbreak resources \\
\hline
FR-C & Stats/\allowbreak Level/\allowbreak Slots & CharacterModule + ConfigModule (milestone slots, công thức chỉ số data-driven) \\
\hline
FR-D & Class System & CharacterModule/\allowbreak Progression service (unlock/\allowbreak switch), điều kiện theo stat/\allowbreak trait/\allowbreak achievement \\
\hline
FR-E & Inventory/\allowbreak Economy & InventoryModule (stack, capacity, currency, equip/\allowbreak enhance), tham chiếu CONFIG\_ITEM \\
\hline
FR-F & Combat/\allowbreak Modes & CombatModule + RealtimeGateway (join instance, tick, snapshot, end-session reward/\allowbreak checkpoint) \\
\hline
FR-G & Gacha & GachaModule (banner/\allowbreak rate/\allowbreak pity/\allowbreak duplicate/\allowbreak history), NPC\_COLLECTION cập nhật bền vững \\
\hline
FR-H & Island \& NPC & IslandModule (build/\allowbreak upgrade/\allowbreak timer/\allowbreak resources), NpcModule (assign worker, job state) \\
\hline
FR-I & Admin & AdminModule (ban/\allowbreak unban, mail, logs, event config), audit log \\
\hline
\end{tabularx}
...
\begin{itemize}
    \item REST dùng để xác thực và chọn profile/\allowbreak character;
    \item session runtime lưu \texttt{profile\_id}/\texttt{character\_id} để WebSocket có thể xác định bối cảnh phiên;
    \item token có hạn và có thể revoke khi cần (ban/\allowbreak unban).
\end{itemize}

\paragraph{(2) Reward delivery và cập nhật inventory}
Luồng thưởng sau phiên (tower/\allowbreak dungeon) dễ phát sinh lỗi cấp trùng nếu người chơi retry hoặc mất kết nối. Thiết kế giảm rủi ro bằng:
\begin{itemize}
    \item server-authoritative: client không tự ghi reward;
    \item cập nhật inventory theo quy tắc stack/\allowbreak capacity và tham chiếu config item;
    \item đề xuất idempotency và business log để đối soát.
\end{itemize}

\paragraph{(3) Gacha: rate/\allowbreak pity/\allowbreak duplicate}
Luồng gacha có yêu cầu “đúng xác suất theo cấu hình” và “không cấp trùng”. Thiết kế đáp ứng bằng:
\begin{itemize}
    \item rates/\allowbreak banners là game configuration (tĩnh) và có thể quản trị bằng pipeline;
...
\paragraph{(4) Island build/\allowbreak upgrade và worker assignment}
...
\begin{itemize}
    ...
    \item island/\allowbreak resources và buildings nhúng trong cùng thực thể để truy xuất nhất quán.
\end{itemize}
...
\begin{itemize}
    ...
    \item giới hạn tần suất thao tác nhạy cảm (login/\allowbreak gacha) và kiểm soát spam input realtime.
\end{itemize}
...
\begin{itemize}
    ...
    \item cập nhật tiến trình (inventory/\allowbreak gacha/\allowbreak island) định hướng atomic ở mức document;
    \item đề xuất idempotency key cho các giao dịch dễ retry (reward/\allowbreak gacha/\allowbreak build upgrade).
\end{itemize}

Hạn chế còn lại: ở mức thiết kế, cơ chế idempotency và transaction log mới dừng ở nguyên tắc, chưa có đặc tả schema log/\allowbreak delta chi tiết cho từng giao dịch. Đây là phần cần đặc tả thêm khi chuyển sang hiện thực.
...
\begin{itemize}
    ...
    \item Redis được định hướng dùng cho cache/\allowbreak pub-sub khi scale ngang nhiều instance;
...
\begin{itemize}
    ...
    \item chưa có tiêu chí tick-rate/\allowbreak bandwidth cụ thể theo mục tiêu tải.
\end{itemize}
Các hạn chế này phù hợp với phạm vi giai đoạn 1 (thiết kế), và sẽ được xử lý khi nâng mức triển khai/\allowbreak benchmark.
...
\begin{itemize}
    \item module hoá theo phân hệ rõ ràng (Auth/\allowbreak Profile/\allowbreak Character/\allowbreak Inventory/\allowbreak Combat/\allowbreak Gacha/\allowbreak Island/\allowbreak Admin);
    \item cross-cutting service cho validation, error handling, logging;
    \item cấu hình data-driven giúp thay đổi nội dung/\allowbreak thông số mà không sửa code lõi.
\end{itemize}

Điểm cần chú ý: khi số lượng phân hệ tăng, cần kiểm soát phụ thuộc chéo (đặc biệt giữa Progression/\allowbreak Inventory/\allowbreak Gacha) bằng cách chuẩn hoá service interface và event nội bộ.
...
\begin{itemize}
    \item request log (latency/\allowbreak status/\allowbreak user context);
    \item business log (reward/\allowbreak currency/\allowbreak gacha/\allowbreak admin actions);
    \item realtime log (join/\allowbreak leave room/\allowbreak disconnect).
\end{itemize}
...
\begin{itemize}
    \item \textbf{Giảm hard-code}: các thông số (item/\allowbreak building/\allowbreak rate/\allowbreak formula) được thay đổi qua TSV.
...
\begin{itemize}
    \item \textbf{Sai schema/\allowbreak kiểu dữ liệu}: cần validation khi import và báo lỗi rõ ràng theo dòng/\allowbreak cột.
    \item \textbf{Tham chiếu chéo sai}: cần kiểm tra referential integrity (itemRefId/\allowbreak buildingId/\allowbreak skillId).
...
\subsection{Đánh giá UI/\allowbreak UX theo định hướng pixel art ở mức hệ thống}
...
\begin{itemize}
    \item chuẩn hoá trạng thái UI (loading/\allowbreak error/\allowbreak disabled) thông qua mã lỗi nhất quán;
    \item ưu tiên payload ``delta'' hoặc snapshot nhỏ gọn cho HUD combat;
    \item phân tách màn hình meta (inventory/\allowbreak gacha/\allowbreak island) khỏi HUD combat để không làm gián đoạn nhịp chơi.
\end{itemize}
...
\renewcommand{\arraystretch}{1.2}
\setlength{\tabcolsep}{6pt}
\begin{tabularx}{\textwidth}{|L{3.3cm}|L{2.0cm}|Y|L{4.2cm}|}
\hline
\textbf{Rủi ro} & \textbf{Mức} & \textbf{Mô tả} & \textbf{Giảm thiểu} \\
\hline
Cấp trùng reward/\allowbreak gacha & Cao &
Retry do mạng/\allowbreak mất kết nối có thể tạo giao dịch lặp &
...
Cập nhật hai chiều, validate trước khi assign/\allowbreak unassign, kịch bản kiểm thử nhất quán \\
\hline
Sai config gây mất cân bằng/\allowbreak lỗi runtime & Cao &
...
\hline
Realtime latency/\allowbreak jitter làm mất cảm giác combat & Trung bình &
Snapshot không ổn định gây giật/\allowbreak khó điều khiển &
Tick-rate hợp lý, snapshot tối thiểu, client interpolation; chừa chỗ mở rộng prediction/\allowbreak reconciliation \\
\hline
...
Các điểm còn cần đặc tả sâu hơn khi chuyển sang hiện thực gồm: cơ chế idempotency/\allowbreak log cho từng giao dịch, chính sách xử lý overflow inventory, tiêu chí tick-rate/\allowbreak bandwidth cho realtime và đặc tả UI state chi tiết cho các tình huống lỗi mạng.

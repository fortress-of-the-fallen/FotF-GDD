...
Chương này hệ thống hoá các kiến thức cơ sở phục vụ cho việc thiết kế một trò chơi casual strategy trong môi trường trực tuyến. Nội dung tập trung vào hai nhóm chính: (i) đặc trưng của casual strategy và các nguyên lý đồng bộ state theo mô hình client--server; (ii) các khái niệm nền tảng của Game Design gồm vòng lặp chơi (loop), hệ thống tiến trình (progression), thiết kế chiến đấu, thiết kế thế giới, kinh tế trong game, UX/\allowbreak UI và cân bằng thông số\cite{adams-game-design,schell-art}. Các khái niệm ở chương này đóng vai trò ``khung lý thuyết'' để liên kết xuyên suốt sang phần phân tích yêu cầu (Chương~5) và phần thiết kế hệ thống (Chương~7).

Trong bản hiện tại của đề tài, một số cơ chế trong chương này đã được áp dụng trực tiếp trong runtime (xác thực, quest, battle session, reward, shop purchase, đồng bộ player state). Một số cơ chế còn lại được giữ ở mức tham chiếu thiết kế để định hướng mở rộng dài hạn (ví dụ các nhánh build chuyên sâu).

\subsection{Luật chơi cốt lõi trong bản hiện tại}

\textbf{Định vị thể loại:} \textit{Fortress of the Fallen} được triển khai theo hướng \textbf{casual strategy}, tập trung vào vòng trận ngắn và quyết định chiến thuật dễ tiếp cận.

\textbf{Flow gameplay chính (battle-first):}
\begin{enumerate}
    \item Từ lobby, người chơi bắt đầu \textbf{tìm trận/matching}.
    \item Hệ thống chuyển qua bước \textbf{roulette} trước khi vào trận.
    \item Người chơi vào \textbf{battle} và thực hiện thao tác chiến đấu trong phiên.
    \item Hệ thống \textbf{chốt kết quả thắng/thua} theo điều kiện của phiên trận.
    \item Kết quả được dùng để \textbf{trả thưởng và đồng bộ hậu trận} (currency, progression, quest progress).
    \item Người chơi quay về lobby và tiếp tục các nhánh tính năng như \textbf{quest, upgrade, shop}.
\end{enumerate}

\textbf{Nguyên tắc dữ liệu:} trạng thái sau trận được xác nhận theo backend response; client chỉ hiển thị và cập nhật theo dữ liệu đã chốt để giữ nhất quán.

% ===============================================================
\subsection{Kiến thức nền tảng về casual strategy và môi trường theo phiên}

\subsubsection{Đặc điểm của casual strategy}

Casual strategy trong phạm vi đề tài kết hợp vòng trận ngắn, quyết định chiến thuật đơn giản và tiến trình nâng cấp dài hạn. Các đặc điểm cốt lõi thường gặp\cite{adams-game-design,schell-art}:
...
\subsubsection{Hệ thống nhiều phiên trong casual strategy}

Với casual strategy online, bài toán kỹ thuật trung tâm là đồng bộ trạng thái trong điều kiện mạng không ổn định: độ trễ (latency), jitter, mất gói, chênh lệch tốc độ khung hình giữa các máy. Thực tế triển khai thường dựa trên mô hình \textbf{server authoritative}\cite{moriarty-networked,gaffer-networking}.
...
Để gameplay cảm giác ``theo phiên'' dù có latency, các kỹ thuật nền tảng thường phối hợp\cite{moriarty-networked,gaffer-networking}:
\begin{itemize}
...
Game Design mô tả cách trò chơi vận hành: người chơi làm gì, vì sao họ muốn làm điều đó, và trò chơi phản hồi như thế nào\cite{adams-game-design,schell-art}. Một casual strategy online thường được phân tích theo các lớp: vòng lặp chơi, tiến trình nhân vật, chiến đấu, nội dung/\allowbreak không gian, kinh tế phần thưởng và UX/\allowbreak UI.

% ---------------------------------------------------------------
\subsubsection{Core Loop và Meta Loop}

\paragraph{Core Loop} là vòng lặp hành vi ngắn hạn lặp lại liên tục. Với casual strategy, core loop thường xoay quanh: (i) tham gia giao tranh; (ii) nhận phần thưởng; (iii) dùng phần thưởng để mạnh lên; (iv) quay lại nội dung khó hơn\cite{adams-game-design}.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.80\textwidth]{Images/CoreLoop.png}
    \caption{Ví dụ mô hình hoá Core Loop trong casual strategy}
    \label{fig:core-loop}
\end{figure}
...
Progression là ``xương sống'' của casual strategy: quyết định tốc độ mạnh lên, cách người chơi ra quyết định build và cách nội dung được mở khoá theo thời gian\cite{adams-game-design,schell-art}. Trong phạm vi kiến thức nền tảng của đề tài, hệ thống tiến trình được trình bày theo ba lớp: (i) chỉ số nền (attributes/\allowbreak stats); (ii) tăng cấp và mở slot kỹ năng; (iii) tiến trình lớp nghề nghiệp (class) và điều kiện mở khoá.
...
\hline
CHA & Sức hút/\allowbreak khả năng dẫn dắt; dùng làm biến điều kiện cho mở khoá, phối hợp đội hình và một số hướng build đặc thù. \\
\hline
\end{tabular}
\end{table}
...
\begin{itemize}
    \item \textbf{Học kỹ năng} (thu thập từ sách, nhiệm vụ, phần thưởng trận, \ldots).
    \item \textbf{Trang bị kỹ năng} (bị giới hạn bởi slot, buộc người chơi ra quyết định).
\end{itemize}
Trong kịch bản tham chiếu mở rộng của đề tài: level tối đa 100; mỗi level cho +5 điểm chỉ số để phân bổ; các mốc level mở dần slot kỹ năng thường và slot kỹ năng kết hợp (combo).
...
\paragraph{Class và cơ chế mở khoá theo điều kiện}
Hệ class là lớp meta-progression giúp người chơi định hình vai trò và phong cách chơi. Một mô hình thường gặp:
\begin{itemize}
    \item \textbf{Class} cung cấp kỹ năng nghề nghiệp, cơ chế chiến đấu đặc thù và nhánh thăng tiến (định hình lối chơi).
\end{itemize}
Trong thiết kế tham chiếu, class được phân tầng (Basic/\allowbreak Intermediate/\allowbreak Advanced/\allowbreak Legendary/\allowbreak Hidden) với điều kiện mở khoá dạng \textbf{ngưỡng chỉ số} (STR/\allowbreak DEX/\allowbreak INT/\ldots), \textbf{trait đặc biệt} (Karma/\allowbreak Affinity/\allowbreak Luck/\allowbreak Resistance) và \textbf{thành tựu hành vi} (đối kháng, mốc tiến trình, \ldots). Cách thiết kế này biến ``mở class'' thành mục tiêu gameplay thay vì chỉ là lựa chọn menu, đồng thời cho phép hệ thống nội dung gắn với điều kiện mở khoá.

% ---------------------------------------------------------------
\subsubsection{Combat Design (Thiết kế chiến đấu)}

Combat là trung tâm trải nghiệm của casual strategy; mọi hệ thống tiến trình cuối cùng đều ``đổ'' vào cảm giác chiến đấu\cite{adams-game-design,schell-art}. Vì vậy, thiết kế combat cần đồng thời kiểm soát nhịp độ, độ rõ ràng và độ công bằng.
...
\begin{itemize}
    ...
    \item Chính xác (ACC) cần có vai trò đối trọng với EVA để cân bằng PvE/\allowbreak đối kháng.
...
World/\allowbreak Level design xác định cấu trúc không gian và cách nội dung được phân phối: luồng di chuyển, mật độ giao tranh, checkpoint, nhịp nghỉ\cite{schell-art}. Với casual strategy online, world design còn liên quan tới:
\begin{itemize}
    \item \textbf{Tổ chức phiên chơi}: nội dung dạng phòng (room) hoặc khu (zone); ảnh hưởng tới cách đồng bộ nhiều phiên.
...
Ngoài ra, một bộ UI tối thiểu cho casual strategy online thường bao gồm:
\begin{itemize}
    \item HUD chiến đấu: HP/\allowbreak MP, thanh kỹ năng theo slot, trạng thái cooldown, buff/\allowbreak debuff.
    \item Màn hình nhân vật: chỉ số, phân bổ điểm, class và trait đặc biệt.
...
Cân bằng là quá trình điều chỉnh thông số để đảm bảo trải nghiệm hợp lý\cite{adams-game-design,schell-art}. Với casual strategy có hệ chỉ số và slot kỹ năng, các trục cân bằng thường gặp:

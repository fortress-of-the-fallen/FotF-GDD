...
\begin{itemize}
    ...
    \item \textbf{Chiến đấu gắn với animation}: logic đòn đánh phụ thuộc vào các mốc thời gian của animation (khi nào hitbox bật/tắt, khi nào có i-frame, thời gian hồi chiêu/khôi phục).
    ...
    \item \textbf{Tính công bằng trong va chạm và sát thương}: cần quy ước rõ về hitbox/hurtbox, phạm vi hiệu lực và điều kiện trúng đòn nhằm giảm cảm giác ``bị oan''.
\end{itemize}
...
\begin{enumerate}
    ...
    \item \textbf{Active}: thời gian hitbox có hiệu lực, đòn có thể gây sát thương/hiệu ứng.
...
\begin{itemize}
    \item \textbf{Client} thu thập input, dự đoán (prediction) ở mức hiển thị và render hình ảnh/âm thanh.
...
Server chạy theo chu kỳ mô phỏng (tick). Ở mỗi tick, server có thể thực hiện: nhận input, cập nhật trạng thái, xử lý va chạm/chiến đấu, sau đó gửi snapshot cho client\cite{moriarty-networked}. Tick-rate cao cho phản hồi tốt hơn nhưng tăng chi phí CPU/băng thông; tick-rate thấp tiết kiệm hơn nhưng đòi hỏi client xử lý nội suy tốt để tránh giật.
...
\paragraph{Meta Loop} là lớp động lực dài hạn (giữ chân người chơi): tăng cấp, mở khoá hệ thống, sưu tầm trang bị/kỹ năng, tối ưu hoá build, tham gia nội dung endgame\cite{adams-game-design}. Meta loop cần có các \textbf{mốc rõ ràng} để tạo cảm giác tiến bộ, đồng thời đủ không gian cho người chơi cá nhân hoá (nhiều hướng build khác nhau).
...
\begin{itemize}
    \item \textbf{Primary Attributes}: người chơi phân bổ trực tiếp (ví dụ STR/DEX/CON/INT/WIS/CHA).
    \item \textbf{Secondary Stats}: suy ra từ primary (HP, MP, tốc độ, chí mạng, chính xác, né tránh, tấn công/phòng thủ, \ldots).
\end{itemize}
...
SPD = s_1\cdot DEX
\]
Việc tách \textit{hệ số} khỏi logic cho phép cân bằng mà không cần thay đổi mã nguồn, đồng thời hỗ trợ thử nghiệm nhiều ``đường cong sức mạnh'' (linear/exponential) ở giai đoạn tinh chỉnh\cite{adams-game-design}.
...
\begin{itemize}
    \item \textbf{Karma}: trục lựa chọn/định hướng hành vi, có thể ảnh hưởng điều kiện mở khoá class và nội dung.
    \item \textbf{Affinity}: liên kết nguyên tố/hệ (Light/Dark/Nature/\ldots), dùng cho phân nhánh class/kỹ năng và kháng.
    ...
    \item \textbf{Resistance}: sức đề kháng/kháng hiệu ứng, giảm cảm giác bị ``khống chế liên hoàn'' trong combat.
\end{itemize}
...
\paragraph{Race/Class và cơ chế mở khoá theo điều kiện}
Hệ race/class là lớp meta-progression giúp người chơi định hình vai trò và phong cách chơi. Một mô hình thường gặp:
\begin{itemize}
...
\end{itemize}
Trong thiết kế tham chiếu, class được phân tầng (Basic/Intermediate/Advanced/Legendary/Hidden) với điều kiện mở khoá dạng \textbf{ngưỡng chỉ số} (STR/DEX/INT/\ldots), \textbf{trait đặc biệt} (Karma/Affinity/Luck/Resistance) và \textbf{thành tựu hành vi} (PvP, Arena, chỉ huy NPC, \ldots). Cách thiết kế này biến ``mở class'' thành mục tiêu gameplay thay vì chỉ là lựa chọn menu, đồng thời cho phép hệ thống nội dung gắn với điều kiện mở khoá.
...
\begin{itemize}
    \item Có hành động nhanh (đánh thường/dash) để duy trì nhịp.
    \item Có hành động mạnh nhưng rủi ro (kỹ năng nặng, hồi chiêu dài) để tạo quyết định.
    \item Có cơ chế phòng thủ/cơ động (dodge/i-frame) nhưng bị giới hạn tài nguyên để tránh lạm dụng.
\end{itemize}
...
\begin{itemize}
    ...
    \item Chính xác (ACC) cần có vai trò đối trọng với EVA để cân bằng PvE/PvP.
    \item HP/Resistance tăng sống sót nhưng cần đi kèm trade-off (mất sát thương hoặc giảm cơ động).
\end{itemize}
...
World/Level design xác định cấu trúc không gian và cách nội dung được phân phối: luồng di chuyển, mật độ giao tranh, checkpoint, nhịp nghỉ\cite{schell-art}. Với Action RPG online, world design còn liên quan tới:
\begin{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Hút (sinks)}: nâng cấp trang bị, chế tạo, tiêu hao vật phẩm, phí sửa chữa/duy trì, các cơ chế RNG.
\end{itemize}
Thiết kế kinh tế tốt đảm bảo người chơi luôn có mục tiêu chi tiêu hợp lý, tránh tích luỹ vô hạn hoặc cạn kiệt quá nhanh.

% ---------------------------------------------------------------
\subsubsection{UX/UI Design trong game và định hướng phong cách Pixel Art}

UX/UI trong game có nhiệm vụ: truyền tải trạng thái, hỗ trợ ra quyết định nhanh và tạo phản hồi nhất quán\cite{schell-art}. Với định hướng đồ hoạ \textbf{pixel art}, các nguyên tắc UI cần nhấn mạnh tính \textbf{đọc được} (readability) và \textbf{rõ trạng thái}:

\begin{itemize}
    \item \textbf{Lưới pixel và căn chỉnh}: UI nên bám lưới, viền/thanh máu dùng bội số pixel để tránh mờ/nhòe.
    \item \textbf{Tương phản cao}: chữ/icon cần tương phản nền đủ lớn; hạn chế gradient mịn gây giảm độ sắc.
    \item \textbf{Phân lớp thông tin}: trong combat chỉ hiển thị thông tin thiết yếu (HP/MP/cooldown/buff-debuff), thông tin chi tiết chuyển sang màn hình nhân vật.
    \item \textbf{Phản hồi hành động}: tấn công trúng, bị trúng, nhặt vật phẩm, nâng cấp thành công/thất bại cần có phản hồi hình--âm rõ ràng, nhất quán.
\end{itemize}

Ngoài ra, một bộ UI tối thiểu cho Action RPG online thường bao gồm:
\begin{itemize}
    \item HUD chiến đấu: HP/MP, thanh kỹ năng theo slot, trạng thái cooldown, buff/debuff.
    \item Màn hình nhân vật: chỉ số, phân bổ điểm, class/race, trait đặc biệt.
    \item Inventory/Equipment: quản lý slot, trang bị, vật phẩm tiêu hao.
    \item Menu xã hội/cơ bản: bạn bè, party (nếu có), thiết lập (âm lượng, ngôn ngữ).
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Power Budget}: tổng sức mạnh được phân bổ qua chỉ số, trang bị, kỹ năng; cần kiểm soát để combo/skill không vượt ngân sách.
\end{itemize}

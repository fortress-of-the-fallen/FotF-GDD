...
Trong bản hiện tại của đề tài, một số cơ chế trong chương này đã được áp dụng trực tiếp trong runtime (xác thực, quest, battle session, reward, đồng bộ player state). Một số cơ chế còn lại được giữ ở mức tham chiếu thiết kế để định hướng mở rộng dài hạn (ví dụ các nhánh build chuyên sâu).
